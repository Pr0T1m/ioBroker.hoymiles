...
let IP = "0.0.0.0"; //DMXface IP address
...
adapter.on('ready', function() {
    ...
    if (TIMING > 10080) { TIMING = 10080; }

    adapter.log.info("DMXfaceXP " + IP + " Port:" + PORT + " DMXchannels:" + DMX_CHANNELS_USED);
    /*
    ...
    if (PW_REQUEST_LIST.length > 0) { //Check for content
        ...
        for (i = 0; i < BUFF.length; i++) { //Check the elements vor valid entries and add to list 
            ...
            var CB = BB.indexOf(')');
            if (CB > CA) { // found  wattage e.g.(100)
            ...
            if (RXdata.length > 8) {
                ...
                if (EX_MINMAX_TRACKING) {
                    ...
                    adapter.getState(exNAME + "_min", (err, state) => {
                        if (state == null) { adapter.setState(exNAME + "_min", exFLOAT); } else {
                            if (exFLOAT < state.val) { adapter.setState(exNAME + "_min", exFLOAT); }
                        }
                    });
                    var MAX_CURRENT;
                    adapter.getState(exNAME + "_max", (err, state) => {
                        if (state == null) { adapter.setState(exNAME + "_max", exFLOAT); } else {
